...
<div class="card card-primary card-outline p-2">
    <div class="card-header">
        <h3 class="card-title">Gacetas</h3> <!-- Agregar un título a la tarjeta -->
    </div>
    <div class="card-body">
        <table id="example" class="table table-striped table-bordered center-table" style="width:100%">
            ...
            <tbody>
                @foreach($gasetas as $gaseta)
                    <tr>
                        <td>{{ $gaseta->id }}</td>
                        <td>{{ $gaseta->nombre }}</td>
                        <td>{{ $gaseta->fecha_importacion }}</td>
                        <td>
                            <a href="{{ Storage::url('public/' . $gaseta->ruta) }}" class="btn btn-success btn-sm" target="_blank">
                                <i class="fas fa-eye"></i>
                            </a>
                        </td>
                        <td>
                            <button type="button" class="btn btn-info btn-sm" onclick="printPDF('{{ Storage::url('public/' . $gaseta->ruta) }}')">
                                <i class="fas fa-print"></i>
                            </button>
                        </td>
                        <td>
                            <a href="{{ route('gasetas.download', $gaseta->nombre) }}" class="btn btn-warning btn-sm">
                                <i class="fa-solid fa-file-pdf"></i>
                            </a>
                        </td>
                        <td>
                            <form action="{{ route('gasetas.destroy', $gaseta->nombre) }}" method="POST" style="display:inline;" onsubmit="confirmDelete(event)">
                                @csrf
                                @method('DELETE')
