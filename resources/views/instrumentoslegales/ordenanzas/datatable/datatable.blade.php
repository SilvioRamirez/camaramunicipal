...
<div class="card card-primary card-outline p-2">
    <div class="card-header">
        <h3 class="card-title">Ordenanzas</h3> <!-- Agregar un título a la tarjeta -->
    </div>
<div class="card-body">
    ...
    <tbody>
        @foreach($ordenanzas as $ordenanza)
            <tr>
                ...
                <td>{{ $ordenanza->fecha_importacion }}</td>
                <td>
                    <a href="{{ Storage::url('public/' . $ordenanza->ruta) }}" class="btn btn-success btn-sm" target="_blank"><i class="fas fa-eye"></i> Ver</a>                                  
                </td>
                <td>
                    <button type="button" class="btn btn-info btn-sm" onclick="printPDF('{{ Storage::url('public/' . $ordenanza->ruta) }}')">
                        <i class="fas fa-print"></i> 
                    </button>
                </td>
                <td>  
                    <a href="{{ route('ordenanzas.download', $ordenanza->nombre) }}" class="btn btn-warning btn-sm"><i class="fa-solid fa-file-pdf"></i> </a>
                </td>
                <td>
                    <form action="{{ route('ordenanzas.destroy', $ordenanza->nombre) }}" method="POST" style="display:inline;" onsubmit="confirmDelete(event)">
                        @csrf
                        @method('DELETE')
