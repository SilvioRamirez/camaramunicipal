...
<div class="card card-primary card-outline p-2">
    <div class="card-header">
        <h3 class="card-title">Especiales</h3> <!-- Cambiado a Especiales -->
    </div>
    <div class="card-body">
        <table id="example" class="table table-striped table-bordered">
            ...
            <tbody>
                @foreach($especiales as $especial)
                    <tr>
                        <td>{{ $especial->id }}</td> <!-- Cambiado a especiales -->
                        <td>{{ $especial->nombre }}</td> <!-- Cambiado a especiales -->
                        <td>{{ $especial->fecha_sesion }}</td> <!-- Cambiado a especiales -->
                        <td>{{ $especial->fecha_importacion }}</td> <!-- Cambiado a especiales -->
                        <td>
                            <a href="{{ Storage::url($especial->ruta) }}" class="btn btn-success btn-sm" target="_blank">
                                <i class="fas fa-eye"></i>
                            </a>
                        </td>
                        <td>
                            <button type="button" class="btn btn-info btn-sm" onclick="printPDF('{{ Storage::url($especial->ruta) }}')">
                                <i class="fas fa-print"></i>
                            </button>
                        </td>
                        <td>
                            <a href="{{ route('especiales.download', $especial->nombre) }}" class="btn btn-warning btn-sm">
                                <i class="fa-solid fa-file-pdf"></i>
                            </a> <!-- Cambiado a especiales -->
                        </td>
                        <td>
                            <form action="{{ route('especiales.destroy', $especial->nombre) }}" method="POST" style="display:inline;" onsubmit="confirmDelete(event)">
                                @csrf
                                @method('DELETE')
